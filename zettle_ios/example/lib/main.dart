...
import 'package:zettle_ios/zettle_models.dart';
...
class _MyAppState extends State<MyApp> {
  final _zettleIosPlugin = ZettleIos();
  ...
  Future<void> initialize() async {
    ...
    // * -- Init zettle plugin
    await _zettleIosPlugin.initialize(
        iosClientId: zettleClientID, redirectUrl: zettleRedirectUrl!);
  }

  Future<void> login() async {
    await _zettleIosPlugin.login();
  }

  Future<void> logout() async {
    await _zettleIosPlugin.logout();
  }

  Future<void> showSettings() async {
    await _zettleIosPlugin.showSettings();
  }
  ...
  Future<void> requestPayment(String value, BuildContext ctx) async {
    ...
    final response = await _zettleIosPlugin.requestPayment(request);
  ...
  Future<void> retrievePayment(BuildContext ctx, String value) async {
    final response = await _zettleIosPlugin.retrievePayment(value);
    if (!mounted) return;
    showPaymentInfo(ctx, response);
  }
  ...
  Future<void> refundPayment(
    ...
    final amount = double.tryParse(refundAmount);

    final request =
        ZettleRefundRequest(reference: reference, refundAmount: amount);
    await _zettleIosPlugin.requestRefund(request);
  }
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
        ...
        home: Builder(builder: (context) {
          return Scaffold(
              appBar: AppBar(
                title: const Text('Zettle iOS Plugin'),
