...
import 'package:zettle_android/zettle_android.dart';
import 'package:uuid/uuid.dart';
import 'package:zettle_android/zettle_models.dart';
...
class _MyAppState extends State<MyApp> {
  final _zettleAndroidPlugin = ZettleAndroid();
  ...
  Future<void> initialize() async {
    await _zettleAndroidPlugin.initialize(
        androidClientId: FlutterConfig.get("ZETTLE_CLIENT_ID"),
        redirectUrl:
            "${FlutterConfig.get("ZETTLE_SCHEME")}://${FlutterConfig.get("ZETTLE_HOST")}");
  }

  Future<void> login() async {
    await _zettleAndroidPlugin.login();
  }

  Future<void> logout() async {
    await _zettleAndroidPlugin.logout();
  }

  Future<void> showSettings() async {
    await _zettleAndroidPlugin.showSettings();
  }
  ...
  Future<void> requestPayment(String value, BuildContext ctx) async {
    ...
    final response = await _zettleAndroidPlugin.requestPayment(request);
  ...
  Future<void> retrievePayment(BuildContext ctx, String value) async {
    final response = await _zettleAndroidPlugin.retrievePayment(value);
    if (!mounted) return;
    showPaymentInfo(ctx, response);
  }
  ...
  Future<void> refundPayment(
    ...
    final amount = double.parse(refundAmount);

    final request =
        ZettleRefundRequest(reference: reference, refundAmount: amount);
    await _zettleAndroidPlugin.requestRefund(request);
  }
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
        ...
        home: Builder(builder: (context) {
          return Scaffold(
              appBar: AppBar(
                title: const Text('Zettle Android Plugin'),
